            ...
            <div class="card-body">
                <form id="updateForm" action="{{ route('admin.api.products.update', $product->id) }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="product_code" id="product_code" class="form-control" value="{{ $product->product_code }}" readonly style="background:#f5f5f5;">
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="translator" id="translator" class="form-control" value="{{ $product->translator }}">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="publish_year" id="publish_year" class="form-control" value="{{ $product->publish_year }}">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea style="min-height: 100px; white-space: pre-line;" name="infomations" id="infomations" class="form-control" required>{{ $product->productDescriptions->infomations ?? '' }}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="features">Dịch vụ & Khuyến mãi</label>
                        <textarea style="min-height: 100px; white-space: pre-line;" name="features" id="features" class="form-control" required>{{ $product->productDescriptions->features ?? '' }}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="applications">Dịch vụ của chúng tôi</label>
                        <textarea style="min-height: 100px; white-space: pre-line;" name="applications" id="applications" class="form-control" required>{{ $product->productDescriptions->applications ?? '' }}</textarea>
                    </div>
                    <div class="form-group">
                        <label for="stock">Tồn kho</label>
                        <input type="number" name="stock" id="stock" class="form-control" value="{{ $product->productInventories->first()->quantity ?? rand(99,500) }}" min="0">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <small class="form-text text-muted">Bạn có thể chọn nhiều ảnh. Ảnh mới sẽ được thêm vào sản phẩm.</small>
                    </div>
...
@section('js')
        ...
        $(document).ready(function () {
            ...
            $('#updateForm').submit(function (e) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.success) {
                            // Nếu có dữ liệu ảnh mới trả về, cập nhật lại phần #imagePreview
                            if (response.images) {
                                let html = '<div class="row">';
                                response.images.forEach(function(img) {
                                    html += `<div class="col-6 col-md-3 mb-4 image-container ${img.is_avatar ? 'is-avatar' : ''}" data-id="${img.id}">
                                        <div class="position-relative h-100">
                                            <img src="${img.url}" alt="" class="img-thumbnail w-100 h-100" style="object-fit: cover;">
                                            <button class="btn btn-danger btn-sm delete-image" data-image-id="${img.id}">&times;</button>
                                            ${img.is_avatar ? '<span class="badge bg-primary avatar-badge">Avatar</span>' : `<div class="avatar-actions"><button type="button" class="btn btn-primary btn-sm set-avatar" data-image-id="${img.id}" data-product-id="${$('#updateForm').data('product-id')}">Set as Avatar</button></div>`}
                                        </div>
                                    </div>`;
                                });
                                html += '</div>';
                                ...
                                // Gắn lại sự kiện cho nút xóa và set avatar
                                attachImageEvents();
                            }
                            Swal.fire({
                                title: "Thành công!",
    ...
    <script>
        // Hàm gắn lại sự kiện cho nút xóa và set avatar sau khi cập nhật giao diện ảnh
        function attachImageEvents() {
            // Set avatar
            $('.set-avatar').off('click').on('click', function(e) {
                e.preventDefault();
                const imageId = $(this).data('image-id');
                const productId = $(this).data('product-id');
                const button = $(this);
            ...
            // Delete image
            $('.delete-image').off('click').on('click', function(event) {
                event.preventDefault();
                let imageId = $(this).data("image-id");
                let imageContainer = $(this).closest(".image-container");
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        $(this).off('click').on('click', function(e) {
                            e.preventDefault();
                            window.imagesToDelete = window.imagesToDelete.filter(id => id !== imageId);
                            imageContainer.removeClass('marked-for-deletion');
                            imageContainer.find('img').removeClass('opacity-50');
                            imageContainer.find('.deletion-badge').remove();
                            $(this).removeClass('btn-warning').addClass('btn-danger').text('×');
